...
const ProblemSolve = () => {
  ...
  const buildExecutableCode = (lang: string, source: string) => {
    ...
    const runner = [
      ...
      "__ca_func = None",
      "",
      "# Try to find the function to execute",
      "# 1. Check for Solution class methods",
      "if 'Solution' in globals():",
      "    try:",
      "        __ca_obj = Solution()",
      "        # Look for any public method in Solution",
      "        for __name in dir(__ca_obj):",
      "            if not __name.startswith('_') and callable(getattr(__ca_obj, __name)):",
      "                __ca_func = getattr(__ca_obj, __name)",
      "                break",
      "    except Exception:",
      "        __ca_func = None",
      "",
      "# 2. Find first function defined by user (not imported)",
      "if __ca_func is None:",
      "    __ca_source_lines = []",
      "    try:",
      "        import inspect",
      "        __ca_src = inspect.getsource(lambda: None).split('\\n')[0]",
      "    except:",
      "        pass",
      "    for __name in sorted(globals().keys()):",
      "        obj = globals()[__name]",
      "        if callable(obj) and not __name.startswith('_'):",
      "            try:",
      "                if not hasattr(obj, '__module__') or obj.__module__ == '__main__' or obj.__module__ == '__builtin__':",
      "                    __ca_func = obj",
      "                    break",
      "            except:",
      "                pass",
      "",
      "# 3. Fallback: use any callable that's not a built-in",
      "if __ca_func is None:",
      "    for __name in sorted(globals().keys()):",
      "        obj = globals()[__name]",
      "        if callable(obj) and not __name.startswith('_'):",
